...
{
    //Problem Link: https://www.codechef.com/START128D/problems/MOOCHEF
    int n, l, r; cin >> n >> l >> r;
    vl v(n); efr(x, v) cin >> x;
    int min_hp = 0, max_hp = 0;
    int things = 0;
    fr(i, 0, n, 1)
    {
        if(v[i] >= l and v[i] <= r)
        {
            things++;
            max_hp = max(max_hp, things);
        }
        else
        {
            things--;
            min_hp = min(min_hp, things);
        }
    }

    cout << max_hp << " " << min_hp << el;
}
signed main()
{
#ifndef OJ
